#!/usr/bin/env python3
"""
spread_competition_tick.py - Evaluate spread bot strategies every 30 minutes.

Reads the active spread competition, checks entry/exit conditions for each
...
def update_spread_cycle_state(comp_id):
    ...
    if n >= per:
        ...
        tg_send(
            f"*Spread Cycle {cycle} complete* — all {per} sprints finished.\n"
            f"Review standings and adjust non-profitable bot strategies, then run:\n"
            f"`python3 /root/.openclaw/workspace/spread_cycle_advance.py`"
        )
        print(f"  Spread Cycle {cycle} complete. Telegram alert sent.")
    else:
        with open(CYCLE_STATE, "w") as f:
            json.dump(state, f, indent=2)
...
def main():
    ...
    if not comp_dir:
        print("No active spread competition.")
        return
    ...
    # Refresh price data
    print(f"Spread tick: {comp_id}  {now_iso()[:16]}")
    for pair in pairs:
