#!/usr/bin/env python3
"""
promotion_check.py - Auto-promotion check for Day League.

Called from day_daily_restart.py at each sprint reset.
Reads the most recently archived sprint's results and checks if autobotday
qualifies for promotion (top-3 for 3 consecutive sprints).

On promotion: copies autobotday's strategy.yaml to fleet/{champion}/strategy.yaml.
"""
import json
import os
import shutil
...
TRACKER_DIR         = os.path.join(WORKSPACE, "competition", "day")
TRACKER_PATH        = os.path.join(TRACKER_DIR, "promotion_tracker.json")
FLEET_DIR           = os.path.join(WORKSPACE, "fleet")
SYN_ALERT           = os.path.join(WORKSPACE, "syn_alert.py")

CHALLENGER          = "autobotday"
PROMOTION_THRESHOLD = 3  # consecutive top-3 finishes required
...
def send_alert(message):
    try:
        ...
        print(f"  [promo/day] Alert failed: {e}")


def find_latest_archived_sprint():
    """Return (comp_id, final_score dict) for the most recently archived day sprint."""
    if not os.path.isdir(RESULTS_DIR):
        print("  [promo/day] Results dir not found.")
        return None, None
...
def do_promote(tracker):
    ...
    try:
        ...
        print(f"  [promo/day] Copied {CHALLENGER}/strategy.yaml -> fleet/{champion}/strategy.yaml")
    except Exception as e:
        print(f"  [promo/day] Strategy copy failed: {e}")
        return
    send_alert(
...
def main():
    ...
    print(f"[promotion_check/day] {now}")
    ...
    if rank is None:
        ...
        # Still record so we don't keep checking
