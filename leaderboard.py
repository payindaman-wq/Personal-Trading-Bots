#!/usr/bin/env python3
"""
leaderboard.py - Cumulative leaderboard across all competition sprints.

Reads every archived final_score.json plus the active sprint (if any).
Ranks bots by cumulative P&L. Also tracks sprint wins, podiums, and
a points tally (8 pts 1st, 5 pts 2nd, 3 pts 3rd, 1 pt 4th-8th).

Usage:
  python3 leaderboard.py              # print leaderboard
  python3 leaderboard.py --json       # also dump leaderboard.json
  python3 leaderboard.py --sprint ID  # detail for one sprint
  python3 leaderboard.py --no-live    # exclude active sprint from totals
"""
import os
import sys
import json
...
def load_active_sprint():
    """
    Return a synthetic final_score dict for the active sprint using current
    portfolio stats. Open positions are counted at cost_basis (not current
    market value) since we don't have a live price feed here.
...
def aggregate(sprints):
    """Aggregate sprint results per bot. Returns dict keyed by bot name."""
    bots = {}
...
def print_leaderboard(bots_dict, active_sprint=None):
    ...
    print("  " + "-" * 74)
    print("  Ranked by cumulative P&L across all sprints.")
    print("  PTS: 1st=8 | 2nd=5 | 3rd=3 | 4th-8th=1  |  1ST = sprint wins  |  TOP3 = podiums")
    print()

    if active_sprint and active_sprint.get("rankings"):
        print(f"  -- LIVE SPRINT: {active_sprint['comp_id']} --")
        print(f"  {'RANK':<6} {'BOT':<12} {'EQUITY':>10} {'PNL':>11} {'TRADES':>7} "
              f"{'WIN%':>6} {'OPEN':>5}")
        print("  " + "-" * 60)
...
def print_sprint_detail(sprint):
    ...
    print(f"  Duration: {sprint.get('duration_hours')}h  |  Pairs: {', '.join(sprint.get('pairs', []))}")
    print(f"  {'RANK':<6} {'BOT':<12} {'EQUITY':>10} {'PNL':>11} {'WIN%':>6} "
          f"{'TRADES':>7} {'FEES':>8} {'MAX DD':>7}")
    print("  " + "-" * 70)
    for r in sprint.get("rankings", []):
        pnl_str = fmt_pnl(r["total_pnl_usd"], 11)
...
def main():
    parser = argparse.ArgumentParser(description="Viking Fleet cumulative leaderboard")
    parser.add_argument("--json",    action="store_true",
                        help="Save leaderboard.json alongside output")
    parser.add_argument("--sprint",  metavar="ID",
                        help="Show detailed results for a single sprint ID")
    parser.add_argument("--no-live", action="store_true",
                        help="Exclude the active sprint from cumulative totals")
