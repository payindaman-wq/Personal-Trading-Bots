#!/usr/bin/env python3
"""Initialize a spread trading competition (7-day sprints, hourly candles)."""
import sys
import json
import os
import yaml
...
ALL_BOTS = []  # populated from fleet/spread/ directory
...
def discover_bots():
    """Find all bots with valid strategy.yaml in fleet/spread/."""
...
def main():
    ...
    if not bots:
        print("Error: no swing bots found in fleet/spread/", file=sys.stderr)
        sys.exit(1)

    pairs = args.pairs

    now      = datetime.now(timezone.utc)
    comp_id  = f"spread-{now.strftime('%Y%m%d-%H%M')}"
    comp_dir = os.path.join(WORKSPACE, "competition", "spread", "active", comp_id)
