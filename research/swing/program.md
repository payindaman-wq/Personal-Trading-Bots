```markdown
# ODIN Research Program — Swing Trading Strategy Optimizer
# Effective from Gen 21001 | Incumbent (STORED): Sharpe=1.3445, trades=58
# MIMIR-reviewed 2026-04-19 (v49)

# ════════════════════════════════════════════════════════
# YOUR ONLY JOB: OUTPUT THE YAML BELOW WITH EXACTLY ONE CHANGE
# ════════════════════════════════════════════════════════

# ████████████████████████████████████████████████
# STEP 1 — COPY THIS YAML EXACTLY
# ████████████████████████████████████████████████

# ```yaml
# name: gen21001_e1a
# style: randomly generated
# pairs:
# - BTC/USD
# position:
#   size_pct: 30
#   max_open: 3
#   fee_rate: 0.001
# entry:
#   long:
#     conditions:
#     - indicator: momentum_accelerating
#       period_hours: 48
#       operator: eq
#       value: false
#     - indicator: bollinger_position
#       period_hours: 48
#       operator: eq
#       value: below_lower
#     - indicator: macd_signal
#       period_hours: 48
#       operator: eq
#       value: bullish
#     - indicator: price_change_pct
#       period_hours: 24
#       operator: lt
#       value: -0.5
#   short:
#     conditions:
#     - indicator: momentum_accelerating
#       period_hours: 48
#       operator: eq
#       value: false
#     - indicator: bollinger_position
#       period_hours: 168
#       operator: eq
#       value: above_upper
#     - indicator: macd_signal
#       period_hours: 24
#       operator: eq
#       value: bearish
#     - indicator: price_change_pct
#       period_hours: 24
#       operator: gt
#       value: 0.5
# exit:
#   take_profit_pct: 6.63
#   stop_loss_pct: 1.5
#   timeout_hours: 129
# risk:
#   pause_if_down_pct: 8
#   stop_if_down_pct: 18
#   pause_hours: 48
# ```

# ████████████████████████████████████████████████
# STEP 2 — APPLY EXACTLY ONE CHANGE FROM THE TEST QUEUE
# ████████████████████████████████████████████████
#
# ACTIVE TEST: E1a
#   Change: long price_change_pct value: -0.5 → -1.0
#
# AFTER YOUR CHANGE, the long price_change_pct block must read:
#     - indicator: price_change_pct
#       period_hours: 24
#       operator: lt
#       value: -1.0

# ════════════════════════════════════════════════
# POISON CHECK — verify before submitting
# ════════════════════════════════════════════════
#
# Required values in your output:
#   pairs: [BTC/USD]  (1 pair only)
#   size_pct: 30
#   max_open: 3
#   take_profit_pct: 6.63
#   stop_loss_pct: 1.5
#   timeout_hours: 129
#   long price_change_pct value: -1.0   ← THE ONE CHANGE
#   short price_change_pct value: 0.5
#
# POISON SIGNATURES (your YAML is wrong if you see these):
#   trades=58, Sharpe≈1.34  →  CLONE. price_change_pct is still -0.5. Fix it.
#   trades=0,  Sharpe=0.00  →  You changed more than one thing. Revert extra changes.
#   pairs includes ETH or SOL  →  WRONG BASE. Use BTC/USD only.
#   take_profit_pct=9.5  →  WRONG BASE. Must be 6.63.
#
# VALID: trades 35–70, Sharpe computed (any value), price_change_pct=-1.0

# ════════════════════════════════════════════════
# RESULT CLASSIFICATION
# ════════════════════════════════════════════════
#
# trades=58, Sharpe≈1.34        [CLONE]         → price_change_pct still -0.5
# trades=0,  Sharpe=0.00        [MALFORMED]     → revert; one change only
# trades<35                     [TOO TIGHT]     → E1a dead; move to E1b
# trades 35–70, Sharpe>1.3445   [NEW BEST]      → success; advance to E1b
# trades 35–70, Sharpe≤1.3445   [VALID DISCARD] → E1a tested; move to E1b

# ════════════════════════════════════════════════
# STORED CHAMPION (do not change these)
# ════════════════════════════════════════════════
#
# Sharpe=1.3445, trades=58, WR=43.1%
#
#   pairs:               BTC/USD  (1 pair)
#   size_pct:            30
#   max_open:            3
#   long price_change:   -0.5, lt, period=24   ← E1a changes to -1.0
#   long bollinger:      below_lower, period=48
#   long macd:           bullish, period=48
#   long momentum:       false, period=48
#   short bollinger:     above_upper, period=168
#   short macd:          bearish, period=24
#   short price_change:  0.5, gt, period=24
#   short momentum:      false, period=48
#   take_profit_pct:     6.63
#   stop_loss_pct:       1.5
#   timeout_hours:       129

# ════════════════════════════════════════════════
# TEST QUEUE
# ════════════════════════════════════════════════
#
# ACTIVE:
#   E1a: long price_change_pct → -1.0        ← CURRENT
#
# QUEUED:
#   E1b: long price_change_pct → -0.75       (if E1a too tight)
#   E1c: long price_change_pct → -0.3        (if E1b too tight; loosens entry)
#   E2a: long bollinger period_hours → 36
#   E2b: long bollinger period_hours → 60
#   E3a: short price_change_pct → 1.0
#   E3b: short bollinger period_hours → 72
#   E4a: add rsi lt 35.0 to long conditions
#   E4b: expand to ETH/USD, SOL/USD pairs
#
# SUSPENDED (confirmed no Sharpe gains):
#   take_profit, stop_loss, timeout, pause, stop_if_down

# ════════════════════════════════════════════════
# KEY RULES
# ════════════════════════════════════════════════
#
# • ONE change only. Never touch more than one value.
# • Exit/risk parameters never improve Sharpe. Do not touch them.
# • Entry condition changes are the only proven source of gains.
# • Do NOT add or remove conditions — only change existing values.
```

---