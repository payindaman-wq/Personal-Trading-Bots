#!/usr/bin/env python3
"""
arb_start_cycle.py — Start a new Arb League cycle.

Run at the beginning of each new cycle (after arb_cycle_advance.py completes).
  1. Runs pair health check — auto-replaces any RETIRE pairs
  2. Starts a new 7-day sprint

Usage: python3 arb_start_cycle.py [--dry-run]
"""
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"  ARB START CYCLE  |  {now_str}")
    if args.dry_run:
    ...
    if args.dry_run:
        print("\n[dry-run] Would now start sprint via arb_competition_start.py")
        print(f"{'='*60}")
    ...
    start_cmd = [
        sys.executable,
        os.path.join(WORKSPACE, "arb_competition_start.py"),
    ]
    result = subprocess.run(start_cmd)
    if result.returncode != 0:
        print("  ERROR: arb_competition_start.py failed")
        sys.exit(1)

    print(f"\n{'='*60}")
    print("  Arb cycle started successfully.")
    print(f"{'='*60}\n")
