...
## Session 21 — 2026-04-26 — oscillation cause wired into Tier 1 history

Wired oscillation detection at loki.py:308 into _record_revert. Added _recurse=False guard to prevent stack recursion when the oscillation block calls _record_revert(league, "oscillation", ..., _recurse=True). The oscillation entry captures oscillation_count, prior_champion_id (prior revert cause), current_champion_id (from best_strategy.yaml), and time_window=24h as a JSON detail string. Tier 1 revert history now records 3 distinct causes (service_crash at line 460, metric_degradation at line 487, oscillation at line 345), unblocking Criterion C of Session 20's Tier 2 build readiness audit. Smoke test verified 3-entry write and cleanup. Commit: 12d661e.
...
## Session 30 — 2026-04-26 — commit recovery + push-immediately discipline

Recovered Sessions 19/21/24 commits that were reverted off master by deploy.yml `git reset --hard origin/master` mid-day. Commits 12d661e (Session 21 oscillation), fe3591e (Session 24 yaml untrack), and 850de95 (Session 19 fleet gitignore) had been committed locally on Mother but not pushed before another push fired the deploy reset. Recovery results: 850de95 cherry-picked as c7ba6c3 (gitignore conflict resolved by keeping both fleet + research blocks); 12d661e cherry-picked clean as 4eeccb2; 73c382c cherry-picked as 00a8d3d (memory conflict resolved Session 21 first). 64fce4f (Session 19 memory) already_on_disk — text re-typed by later session. fe3591e (Session 24) already_on_disk — gitignore line, all 5 seed_strategy.yaml files, bootstrap_research_yamls.sh, and best_strategy.yaml untrack state all present in HEAD when audited (only stale .bak/grid yamls remained tracked — unrelated). Underlying race: sessions that commit locally and don't push immediately are at risk of deploy.yml wiping their work on the next push from any source. Process fix: every session must push immediately after commit. See feedback_long_running_services_hold_old_code.md for the disk-vs-runtime parallel; this is the disk-vs-history version of the same class of bug.
