#!/usr/bin/env python3
"""
futures_swing_leaderboard.py - Cumulative leaderboard for Futures Swing league.
"""
...
LB_PATH     = os.path.join(WORKSPACE, "competition", "futures_swing", "futures_swing_leaderboard.json")
    ...
    for entry in sorted(os.listdir(RESULTS_DIR)):
        ...
        if os.path.isfile(score_path):
            ...
            sprints.append({
                ...
                "duration_hours": sd.get("duration_hours", 168),
...
def main():
    parser = argparse.ArgumentParser(description="Futures Swing leaderboard")
    ...
    print(f"\n{'='*72}")
    print(f"  FUTURES SWING LEAGUE LEADERBOARD  --  {now}")
    print(f"  Cycle {cycle.get('cycle',1)} | Sprint {cycle.get('sprint_in_cycle',0)}/{cycle.get('sprints_per_cycle',4)}")
    if active:
    ...
    if args.json:
        out = {
            ...
            "sprints_per_cycle": cycle.get("sprints_per_cycle", 4),
